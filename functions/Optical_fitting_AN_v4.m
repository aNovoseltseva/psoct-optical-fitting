function Optical_fitting_AN_v4(co, cross, s_seg, z_seg, datapath,aip_threshold, mus_depth, bfg_depth, ds_factor, zf_l, ZR, C, sur_offset)
% Fitting scattering and retardance of brain tissue
% co: volume data for co pol
% cross: volume data for cross pol
% s_seg: slice number
% Z_seg: tile number
% datapath: datapath for original data
% aip_threshold: intensity threshold to remove agarose, use same value as
% in OCT_recon.m
% mus_depth: number of z pixels for fitting mus
% bfg_depth: number of z pixels for fitting birefringence
% ds_factor: downsample factor
% zf: focus depth 
% C is constant offset due to noise level
    NA = 0.085; ref_ind = 1.33;
    cd(datapath);
    opts = optimset('Display','off','TolFun',1e-10);
    ds_prefit = 20;
    co1=imresize3(co,[size(co,1), size(co,2)/ds_prefit, size(co,3)/ds_prefit]);
    cross1=imresize3(cross, [size(cross,1), size(cross,2)/ds_prefit, size(cross,3)/ds_prefit]);
    ref=single(sqrt(co1.^2+cross1.^2));
    sur=surprofile2(ref,'PSOCT',1); % find surface, downsample=20
    sur=surprofile_agar(ref,'PSOCT',1); % find surface for agar
    Z_step=3;
    zf_full = imresize(zf_l,20);
    zf_ds = imresize(zf_full, 1/ds_factor);
    zf_l = imresize(zf_full, 1/ds_prefit);
    % sensitivity roll-off correction
    % w=2.2; % sensitivity roff-off constant, w=2.2 for 5x obj, w=2.22 for 10x obj
    % I=rolloff_corr(I,w);

    aip=squeeze(mean(ref(1:110,:,:),1));
    mask=zeros(size(aip));
    mask(aip>aip_threshold)=1;
    mask2=zeros(size(ref));
    for i=1:size(ref,1)
        mask2(i,:,:)=mask;
    end
    vol=mask2.*ref;
    clear mask2
    clear ref
    
    %% Prefit with 2 parameters, zr = 70 um, zf = (zf-surface)*Z_step and adjust according to zf_tilt
    param = zeros(size(vol,2),size(vol,3),2); 
    r2 = zeros(size(vol,2), size(vol,3));
    for i = 1:size(vol,2)
        for j = 1:size(vol,3)
            aline = vol(:,i,j);
            %aline = aline - mean(aline(end-10:end-5));
            start_depth=sur(i,j)+sur_offset;
            fit_depth=min(length(aline)-5,start_depth+mus_depth-1);
            if max(aline) > 0.05
                ydata = double(aline(start_depth:fit_depth)');
                zdata = (sur_offset:length(ydata)+sur_offset-1)*Z_step;
                % *** what is going on with z_f - surface?
                fun_pix = @(p,zdata)double(C + sqrt(p(1).*exp(-2.*p(2).*(zdata)).*(1./(1+((zdata-zf_l(i,j))./ZR).^2)))); 
                lb = [0.00000001 0.00000001 ];
                ub=[20 0.03 ];
                try
                   param(i,j,:) = lsqcurvefit(fun_pix,[0.001 0.001],zdata,ydata,lb,ub,opts);
                   r2(i,j) = 1 - sum((ydata - fun_pix(param(i,j,:),zdata)).^2)/sum((ydata - mean(ydata)).^2);

                catch
                    param(i,j,:)=[0 0 ];
                end
            else
                param(i,j,:) = [0 0 ];
            end
        end           
    end

    % figure; scatter(zdata,ydata);hold on;y=fun_pix(param(i,j,:),zdata);plot(zdata,y)
    figure(1);imagesc(r2); colorbar; caxis([0.8 1]); title(strcat('R2=',num2str(100*nanmean(r2, 'all')) ,' prefit'));
    %imagesc(param(:,:,1)); title('mub'); colorbar; caxis([0 1]);
    %imagesc(1000.*param(:,:,2)); title('mus'); caxis([0 20]); colorbar;
     musPrefit = single(1000.*squeeze(param(:,:,2)));     % unit:mm-1
     maskUsPre=zeros(size(musPrefit));
     maskUsPre(musPrefit>0.2)=1;
     musPrefit(maskUsPre==0)=mean2(musPrefit(maskUsPre==1));
     musPrefit = my_convn(musPrefit,ones(5,5));
     musPrefit = imresize(musPrefit, 20/ds_factor);

    %% fit again with 2 parameters, adjusted zf and zr according to prefit results
    co1=imresize3(co,[size(co,1), round(size(co,2)/ds_factor), round(size(co,3)/ds_factor)]);
    cross1=imresize3(cross, [size(cross,1), round(size(cross,2)/ds_factor), round(size(cross,3)/ds_factor)]);
    ref=single(sqrt(co1.^2+cross1.^2));
    sur=surprofile2(ref,'PSOCT',1); % find surface, downsample=10
    % flatten surface where tissue surface is below or above average
    % tissue surface by two standard deviations
    if mean2(sur)>1
        sur_smooth=sur;
        masksur=ones(size(sur_smooth));
        masksur(sur_smooth>mean2(sur)+2*std2(sur))=0;
        sur_smooth(masksur==0)=mean2(sur_smooth(masksur==1));
        sur_smooth = my_convn(sur_smooth,ones(10,10));
    end
    % masking volume to remove agarose
    aip=squeeze(mean(ref(1:110,:,:),1));
    mask=zeros(size(aip));
    mask(aip>aip_threshold)=1;
    mask2=zeros(size(ref));
    for i=1:size(ref,1) 
        mask2(i,:,:)=mask;
    end
    vol=mask2.*ref;
    clear mask2
    clear ref

    mus_grey=1; % for unindex matched sample, for index matched, use ~0
    mus_ratio=3; % for unindex matched sample, for index matched, use ~5
    % start fitting
    param = zeros(size(vol,2),size(vol,3),4); 
    R_2=zeros(size(vol,2),size(vol,3));
    %%%%%%%%%%%%%%%%%%%%%%%%
    % stop here to check the scattering aline by define i and j to be at
    % the center of the image =87 and plot(aline)
    %i=87;j=87;plot(aline)
    %%%%%%%%%%%%%%%%%%%%%%%%
    for i = 1:size(vol,2)
        for j = 1:size(vol,3)
            %%%%evaluate this section on CW%%%%
            aline = vol(:,i,j);
            start_depth=sur(i,j)+sur_offset;  % *** find surface                                                           
            % depth to start fitting, 2 pixel below surface


            if max(aline) > 0.05                                                               % remove agarose
                ydata = double(aline(start_depth:fit_depth)');
                zdata = (sur_offset:length(ydata)+sur_offset-1)*Z_step;
                param(i,j,3)=zf_ds(i,j);        
                param(i,j,4) = ZR*sqrt(1+(musPrefit(i,j)*zf_ds(i,j)/4)*(NA/ref_ind)^2);    
                % define fitting function
                fun_pix = @(p,zdata)double(C + sqrt(...
                    p(1).*exp(-2.*p(2).*(zdata)).*...                                           % mub and mus variables
                    (1./(1+((zdata-param(i,j,3))...                                             % using corrected zf 
                    ./param(i,j,4)).^2))...                                                     % using mus corrected zr
                    ));  
                % fitting
                lb = [0.00000001 0.00000001 ];
                ub=[20 0.03 ];
                try
                      [param(i,j,1:2), rn] = lsqcurvefit(fun_pix,[0.1 0.006 ],zdata,ydata,lb,ub,opts);
                   %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                   % pause here to check the fitting quality
                   % !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                   R_2(i,j) = 1 - sum((ydata - fun_pix(param(i,j,:),zdata)).^2)/sum((ydata - mean(ydata)).^2);
                catch
                    param(i,j,1:2)=[0 0 ];
                    display(['fitting mus failed at',num2str(i),' ',num2str(j)])
                end
            else
                param(i,j,1:2) = [0 0 ];
            end
            %%%stop here and check the fitting quality %%%
            %figure(1); scatter(zdata,ydata);hold on;y=fun_pix(param(i,j,:),zdata);plot(zdata,y)
        end           
    end
    figure(2);imagesc(R_2); colorbar; caxis([0.8 1]); title(strcat('R2=',num2str(100*mean(R_2,'all', 'omitnan')) ,' fit'));
    %imagesc(param(:,:,1)); title('mub'); colorbar; caxis([0 1]);
    %imagesc(1000.*param(:,:,2)); title('mus'); caxis([0 20]); colorbar;
    %% visualization & save
    R_2=single(R_2);
    tiffname=strcat(datapath,'fitting/vol',num2str(s_seg),'/','R2.tif');
    SaveTiff(R_2,z_seg,tiffname);

    mus = single(1000.*squeeze(param(:,:,2)));     % unit:mm-1
    tiffname=strcat(datapath,'fitting/vol',num2str(s_seg),'/','MUS.tif');
    SaveTiff(mus,z_seg,tiffname);

    mub = single(squeeze(param(:,:,1)));
    tiffname=strcat(datapath,'fitting/vol',num2str(s_seg),'/','MUB.tif');
    SaveTiff(mub,z_seg,tiffname);

    zf = single(squeeze(param(:,:,3)));
    tiffname=strcat(datapath,'fitting/vol',num2str(s_seg),'/','ZF.tif');
    SaveTiff(zf,z_seg,tiffname);

    zr = single(squeeze(param(:,:,4)));
    tiffname=strcat(datapath,'fitting/vol',num2str(s_seg),'/','ZR.tif');
    SaveTiff(zr,z_seg,tiffname);


%% fitting retardance slope
    co1=imresize3(co,[size(co,1), size(co,2)/ds_factor, size(co,3)/ds_factor]);
    cross1=imresize3(cross, [size(cross,1), size(cross,2)/ds_factor, size(cross,3)/ds_factor]);
    ref=single(sqrt(co1.^2+cross1.^2));
    sur=surprofile2(ref,'PSOCT',1); 

    % remove agarose
    aip=squeeze(mean(ref(1:110,:,:),1));
    mask=zeros(size(aip));
    mask(aip>aip_threshold)=1;
    mask2=zeros(size(ref));
    for i=1:size(ref,1)
        mask2(i,:,:)=mask;
    end
    ret=single(atan(cross1./co1));
    vol=ret.*mask2;
    clear mask2
    clear ret

    % fitting
    param = zeros(size(vol,2),size(vol,3),2);
    for i = 1:size(vol,2)
        for j = 1:size(vol,3)
            %i=17;j=22;
            aline = vol(:,i,j);
            start_depth = sur(i,j)+5;
            
            if max(aline) > 0.001
                if ~exist('mus','var')
                    mus = imread(strcat(datapath,'fitting/vol',num2str(s_seg),'/',num2str(z_seg),'_mus.tif'),1);
                end
                
                % Initial fit region
                fit_depth = min(size(aline,1)-5, start_depth+bfg_depth-1);
                ydata = double(aline(start_depth:fit_depth)');
                zdata = (5:(length(ydata)+4))*Z_step;
                
                try
                    param(i,j,:) = fitLinearRegion(zdata, ydata);
                catch
                    param(i,j,:)=[0 0];
                    display(['fitting bfg failed at',num2str(i),' ',num2str(j)])
                end
            end
        end
    end

    %% visualization & save
    % 
    bfg = single(squeeze(param(:,:,1)));     % unit:mm-1
    tiffname=strcat(datapath,'fitting/vol',num2str(s_seg),'/','BFG.tif');
    SaveTiff(bfg,z_seg,tiffname);

    bkg = single(squeeze(param(:,:,2)));
    tiffname=strcat(datapath,'fitting/vol',num2str(s_seg),'/','BKG.tif');
    SaveTiff(bkg,z_seg,tiffname);
end