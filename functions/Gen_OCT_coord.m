function Gen_OCT_coord(datapath, disp, mosaic, pattern, plane1, plane2, plane3)
    %%% ---------------------- %%%
    % Adjusted function to generate coordinates in row-major order
    % Author: Jiarui Yang, Shuaibin Chang (modified)

    %% Define parameters
    numX = mosaic(1); % Number of columns
    numY = mosaic(2); % Number of rows

    %% Generate ImageJ Macro (Unchanged)
    ...
    fprintf(fid_Macro, 'close();\n');
    l = ['run("Grid/Collection stitching", "type=[Grid: snake by columns] order=[Down & Left] grid_size_x=',num2str(numY),' grid_size_y=',num2str(numX),' tile_overlap=10 first_file_index_i=1 directory=', ...
         filepath_rev, [' file_names=Composite-{iiii}.tif output_textfile_name=TileConfiguration.txt ' ...
         'fusion_method=[Linear Blending] regression_threshold=0.01 max/avg_displacement_threshold=50 absolute_displacement_threshold=50 compute_overlap computation_parameters=[Save memory (but be slower)] image_output=[Write to disk] output_directory='], ...
         filepath_rev, '");\n'];
    fprintf(fid_Macro, '%s', l);
    fprintf(fid_Macro, 'run("Quit");\n');
    fclose(fid_Macro);
    command = "xvfb-run -a " + "/projectnb/npbssmic/ns/Fiji/Fiji.app/ImageJ-linux64 --run " + macropath;
...
%     cd(filepath);
%     fileID = fopen(strcat(filepath, 'TileConfiguration.txt'),'w');
