...
resize_factor=4;  %%down sampling by 4
%resize_factor=1;
...
for i=1:ntiles
    ...
    if length(filename0)==1
        ifilePath=strcat(datapath2,filename0(1).name);
...
% use following 3 lines if stitch using 2P coordinates
    ...
    f=strcat(coordpath,'TileConfiguration.registered.txt');
    coord = read_Fiji_coord(f,'Composite');
    if highres==1 %%%%%%%%%%%%%%%% if using 1um pixel size in 2PM or 2um in 2PM
        coord(2,:)=coord(2,:).*1.035/3; %%%%%%%%%%%%%%%% pixel size ratio between 2PM and OCT
        coord(3,:)=coord(3,:).*1.022/3; %%%%%%%%%%%%%%%% pixel size ratio between 2PM and OCT
    else
        coord(2:3,:)=coord(2:3,:).*2/3; %%%%%%%%%%%%%%%% pixel size ratio between 2PM and OCT
    end
end
...
%% save data
save(strcat(datapath2,'volume/ref_1ds',num2str(islice),'.mat'),'Ref','-v7.3');
%save(strcat(datapath2,'volume/ref_1ds',num2str(islice),'.mat'),'Ref','-v7.3');
clear options;
options.big = true; % Use BigTIFF format
options.overwrite = true;
saveastiff(single(Ref), strcat('Ref_BASIC_1ds',num2str(id),'.btf'), options);
%saveastiff(single(Ref), strcat('Ref_BASIC_1ds',num2str(id),'.btf'), options);
clear options;
options.big = false; % Use BigTIFF format
options.overwrite = true;
saveastiff(single(imresize3(Ref,0.4)), strcat('Ref_BASIC_P1ds',num2str(id),'.tif'), options);
