function metric_stitch(target, P2path, datapath,disp,mosaic,pxlsize,islice,pattern,sys,ds,stitch)
%% define grid pattern
...
filename0=dir(strcat(upper(target),'.tif'));%dir('MUS.tif');

filename = strcat(filepath,upper(target),'_flagged.tif');%strcat(filepath,'MUS_flagged.tif');
flagged=0;
for j=1:numX*numY
    if tile_flag(j)>0
        try
            metric = single(imread(filename0(1).name, j));
            if flagged==0
                t = Tiff(filename,'w');
                flagged=1;
            else
            ...
            tagstruct.ImageLength     = size(metric,1);
            tagstruct.ImageWidth      = size(metric,2);
...
%write uncorrected .tif tiles
filename0=strcat(datapath,'fitting/vol',num2str(islice),'/',upper(target),'.tif');
filename0=dir(filename0);
for iFile=1:numTile
    this_tile=iFile;
    try
    metric = double(imread(filename0(1).name, iFile));
    avgname=strcat(datapath,'fitting/vol',num2str(islice),'/',num2str(this_tile),'.mat');
    save(avgname,'metric');  

    metric=single(metric);
    tiffname=strcat(datapath,'fitting/vol',num2str(islice),'/',num2str(this_tile),'_',target,'.tif');
    SaveTiff(metric,1,tiffname);
    catch
        iFile
    end
end
...
for i=1:length(index)
    ...
    if isfile(filename0.name)
        ...
        if strcmp(sys,'PSOCT')
            Mosaic(row,column)=Mosaic(row,column)+metric.*Masque2(row,column); 
        elseif strcmp(sys,'Thorlabs')
            Mosaic(row,column)=Mosaic(row,column)+metric'.*Masque2(row,column);
        end
    end
