...
max_noise = 0.1;
mus_arr = 0.020.*rand(N_iter,1)+0.01; 
...
for i = 1:N_iter
    ...
    %generate noise
    gaus_noise = wgn(fit_depth,1,0)';
    temp = movmean(gaus_noise,5); % slow down the noise to match real data
    noise = max_noise*temp+max_noise/3;
...
[Xsorted,I] = sort(real);
Ysorted = errors(I);
figure(3); plot(Ysorted); ylabel('error, %')
